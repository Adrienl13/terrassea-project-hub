...
const Auth = () => {
  ...
  const inputClass = "w-full text-base font-body bg-transparent border border-border rounded-full px-4 py-3 focus:outline-none focus:border-foreground transition-colors";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-12 md:pt-28 pb-16 px-6">
        <motion.div
          ...
          <div className="flex gap-1 bg-card border border-border rounded-full p-1 mb-8">
            {(["login", "register"] as Mode[]).map((m) => (
              <button
                key={m}
                onClick={() => setMode(m)}
                className={`flex-1 py-3 min-h-[44px] text-xs font-display font-semibold rounded-full transition-colors ${
                  mode === m
