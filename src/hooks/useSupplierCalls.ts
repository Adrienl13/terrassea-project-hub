...
// ── Hook 2: usePartnerLeads (for partners) ─────────────────────────────────────

export function usePartnerLeads(partnerId: string | null | undefined) {
  const queryClient = useQueryClient();

  const {
    data: leads = [],
    isLoading,
  } = useQuery<PartnerLead[]>({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          "id, request_id, score_total, status, created_at, request:request_id(project_title, project_type, project_city, project_country, categories_needed, style_preferences, budget_range, quantity_estimate, timeline, description)",
        )
        .eq("partner_id", partnerId)
        .order("created_at", { ascending: false });

      if (error) {
        console.error("Failed to fetch partner leads:", error.message);
        return [];
      }
      ...
      return (data ?? []).map((m: any) => ({
        ...
        projectTitle: m.request?.project_title ?? "",
        projectType: m.request?.project_type ?? "",
        projectCity: m.request?.project_city ?? null,
        projectCountry: m.request?.project_country ?? null,
        categoriesNeeded: m.request?.categories_needed ?? [],
        stylePreferences: m.request?.style_preferences ?? [],
        budgetRange: m.request?.budget_range ?? null,
        quantityEstimate: m.request?.quantity_estimate ?? null,
        timeline: m.request?.timeline ?? null,
        description: m.request?.description ?? null,
        matchScore: m.score_total,
  ...
  const submitResponseMutation = useMutation({
    mutationFn: async ({
      ...
      const { error } = await supabase.from("pro_service_responses").insert({
        ...
        delivery_weeks: data.delivery_weeks ?? null,
  ...
  return {
    leads,
    isLoading,
    expressInterest: expressInterestMutation.mutateAsync,
    isExpressingInterest: expressInterestMutation.isPending,
    declineLead: declineLeadMutation.mutateAsync,
    ...
    ) => submitResponseMutation.mutateAsync({ requestId, data }),
    isSubmitting: submitResponseMutation.isPending,
  };
}

// ── Hook 3: useTrackCallEvent ──────────────────────────────────────────────────
