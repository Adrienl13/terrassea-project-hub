...
const STYLE_KEYS = [
  { value: "mediterranean", palette: ["#D4603A", "#C4956A", "#E8D5B0", "#4A90A4", "#6B7B5E"], paletteNames: ["Terracotta", "Ocre", "Sand", "Azure", "Olive"] },
  { value: "bistro", palette: ["#C0392B", "#7D2935", "#1A1A1A", "#6B4C1E", "#F5E6D3"], paletteNames: ["Red", "Bordeaux", "Iron", "Dark wood", "Cream"] },
  { value: "natural", palette: ["#8B7355", "#E8DDD3", "#6B7B5E", "#D4C9B8", "#A0856E"], paletteNames: ["Teak", "Linen", "Sage", "Dune", "Clay"] },
  { value: "modern", palette: ["#1A1A1A", "#F5F0EB", "#888888", "#C0C0C0", "#D4C9B8"], paletteNames: ["Black", "Off-white", "Concrete", "Steel", "Stone"] },
  { value: "luxury", palette: ["#1B4D3E", "#1A2456", "#722F37", "#C4956A", "#F5E6D3"], paletteNames: ["Emerald", "Navy", "Bordeaux", "Gold", "Ivory"] },
  { value: "industrial", palette: ["#2D2D2D", "#C4956A", "#C0C0C0", "#5D3A1A", "#F5F0EB"], paletteNames: ["Anthracite", "Bronze", "Steel", "Dark wood", "Off-white"] },
  { value: "coastal", palette: ["#2C5F7C", "#FFFFFF", "#F5E6D3", "#8B7355", "#A8C5D6"], paletteNames: ["Navy", "White", "Sand", "Rope", "Sky"] },
  { value: "tropical", palette: ["#6B7B5E", "#D4A574", "#F5E6D3", "#8B7355", "#2BBCD4"], paletteNames: ["Palm", "Bamboo", "Coconut", "Teak", "Turquoise"] },
];
  ...
  return (
    ...
    >
      <h2 className="font-display text-xl md:text-2xl font-bold text-foreground mb-2">
        {t('projectBuilder.styleStep.question')}
      </h2>
      <p className="text-sm font-body text-muted-foreground mb-8">
        {t('projectBuilder.styleStep.subtitle')}
      </p>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {STYLE_KEYS.map((option) => {
          const isSelected = selectedStyles.includes(option.value);
          const label = t(`projectBuilder.styleStep.${option.value}`);
          const description = t(`projectBuilder.styleStep.${option.value}Desc`);
          return (
            <button
              key={option.value}
      ...
      {selectedStyles.length === 0 && (
        <p className="text-[11px] font-body text-muted-foreground mt-4 text-center">
          {t('projectBuilder.styleStep.multipleHint')}
        </p>
      )}
      ...
      {selectedStyles.length > 0 && (
        <p className="text-[11px] font-body text-muted-foreground mt-4 text-center">
          {selectedStyles.length > 1
            ? t('projectBuilder.styleStep.selectedPlural', { count: selectedStyles.length })
            : t('projectBuilder.styleStep.selected', { count: selectedStyles.length })}
        </p>
      )}
      ...
      <div className="flex items-center justify-between mt-8">
        {onBack ? (
          ...
          >
            ...
            {t('projectBuilder.styleStep.back')}
          </button>
        ) : <div />}
        ...
        {onNext && (
          ...
          >
            {t('projectBuilder.styleStep.reviewBrief')}
            <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
export { STYLE_KEYS as STYLE_OPTIONS };
