...
import { Bell, MessageSquare, Info, CheckCheck, X } from "lucide-react";
...
export default function NotificationBell() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-background border border-border rounded-sm shadow-lg z-50 overflow-hidden">
          {/* Header */}
          ...
          <div className="max-h-72 overflow-y-auto">
            {notifications.length === 0 ? (
              <div className="px-4 py-8 text-center">
                ...
                <p className="text-xs font-body text-muted-foreground">Aucune notification</p>
              </div>
            ) : (
              notifications.slice(0, 15).map(n => {
                ...
                return (
                  ...
                  >
                    <div className={`mt-0.5 p-1.5 rounded-sm ${!n.read_at ? "bg-blue-100" : "bg-foreground/5"}`}>
                      <Icon className={`h-3 w-3 ${!n.read_at ? "text-blue-600" : "text-muted-foreground"}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={`text-xs font-body leading-tight ${!n.read_at ? "text-foreground font-semibold" : "text-muted-foreground"}`}>
                        {n.title}
                      </p>
                      {n.body && (
                        <p className="text-[10px] font-body text-muted-foreground mt-0.5 truncate">{n.body}</p>
                      )}
                    </div>
